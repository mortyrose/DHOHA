...
def compute_irtr_my(pl_module, batch):
    ...
    loss_scan = compute_SCAN(
        image_feats,
        text_feats,
        text_lengths,
        add_data,
        pl_module.hparams.config["margin"],
        pl_module.hparams.config["direction"],
        pl_module.hparams.config["lambda_softmax"],
    )

    def masked_mean(feats, mask):
        mask = mask.to(feats.device).unsqueeze(-1).float()
        return (feats * mask).sum(dim=1) / mask.sum(dim=1).clamp(min=1.0)

    img_global_final = F.normalize(image_feats.mean(dim=1), p=2, dim=-1)
    txt_global_final = F.normalize(masked_mean(text_feats, input_mask), p=2, dim=-1)

    logits = img_global_final @ txt_global_final.t()
    logits = logits / 0.07
    ...
    img_global_1 = F.normalize(image_feats_1.mean(dim=1), p=2, dim=-1)
    img_global_2 = F.normalize(image_feats_2.mean(dim=1), p=2, dim=-1)

    txt_global_1 = F.normalize(masked_mean(text_feats_1, input_mask), p=2, dim=-1)
    txt_global_2 = F.normalize(masked_mean(text_feats_2, input_mask), p=2, dim=-1)

    def pairwise_orth_loss(a, b):
        return (F.cosine_similarity(a, b, dim=-1) ** 2).mean()

    loss_orth_img = (
        pairwise_orth_loss(img_global_final, img_global_2) +
        pairwise_orth_loss(img_global_final, img_global_1) +
        pairwise_orth_loss(img_global_2, img_global_1)
    ) / 3.0

    loss_orth_txt = (
        pairwise_orth_loss(txt_global_final, txt_global_2) +
        pairwise_orth_loss(txt_global_final, txt_global_1) +
        pairwise_orth_loss(txt_global_2, txt_global_1)
    ) / 3.0

    loss_orth = loss_orth_img + loss_orth_txt

    batch_size = image_feats.size(0)
    global_weight = pl_module.hparams.config.get("global_contrastive_weight", 0.1)
    orth_weight = pl_module.hparams.config.get("orth_weight", 0.01)

    loss = (
        loss_scan
        + global_weight * loss_global_final * batch_size
        + orth_weight * loss_orth * batch_size
    )
