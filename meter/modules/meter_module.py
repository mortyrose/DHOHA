...
class METERTransformerSS(pl.LightningModule):
    def __init__(self, config):
        ...
        set_requires_grad(self.vit_model, False)
        set_requires_grad(self.text_transformer, False)
        
        # Optional: record whether we already unfroze (avoid repeated work)
        self._unfroze_backbones = False
    def on_train_epoch_start(self):
        # unfreeze after 10 epochs: epoch 0-9 frozen, epoch 10+ trainable
        if self.current_epoch == 10:
            set_requires_grad(self.vit_model, True)
            set_requires_grad(self.text_transformer, True)
            self._unfroze_backbones = True

        # Debug print (keep or remove)
        if self.current_epoch in [0, 9, 10, 11]:
            vit_grad = any(p.requires_grad for p in self.vit_model.parameters())
            bert_grad = any(p.requires_grad for p in self.text_transformer.parameters())
            print(f"[epoch {self.current_epoch}] vit_grad={vit_grad}, bert_grad={bert_grad}")
    ...
    def validation_epoch_end(self, outs):
        eval_interval = self.hparams.config.get("eval_interval", 1)

        if self.current_epoch != 0 and self.current_epoch % eval_interval == 0:
            self.eval()
            with torch.no_grad():
                print(f"\n===== Epoch {self.current_epoch} | VAL =====")
                meter_utils.epoch_eval_irtr(self, is_test=False)
                print(f"\n===== Epoch {self.current_epoch} | TEST =====")
                meter_utils.epoch_eval_irtr(self, is_test=True)
        else:
            self.log('best_irtr', 0.0, sync_dist=True)
