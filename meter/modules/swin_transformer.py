...
class DeformableWindowAttention(nn.Module):
    """
    Window attention + DAT-style offset sampling + continuous RPB.
    """
    def __init__(
        self,
        dim,
        ...
        self.conv_offset = nn.Sequential(
            ...
            LayerNormProxy(self.head_dim),
            nn.GELU(),
            nn.Conv2d(self.head_dim, 2, kernel_size=1, stride=1, padding=0, bias=True),
        )
    ...
    def _get_window_grid(self, B, dtype, device): 
        Wh, Ww = self.window_size
        ys = torch.linspace(-1.0, 1.0, Wh, dtype=dtype, device=device)
        xs = torch.linspace(-1.0, 1.0, Ww, dtype=dtype, device=device)
        yy, xx = torch.meshgrid(ys, xs)  
        grid = torch.stack((yy, xx), dim=-1)   # Wh, Ww, 2 ; (y, x)
        return grid.unsqueeze(0).expand(B, -1, -1, -1)  # B, Wh, Ww, 2

    def _get_continuous_rpb(self, B_, pos):
        """
        pos: [B_ * num_heads, Wh, Ww, 2]  in [-1, 1]
        return: [B_, num_heads, N, N]
        """
        Wh, Ww = self.window_size
        N = self.N

        q_grid = self._get_window_grid(B_ * self.num_heads, pos.dtype, pos.device)
        q_grid = q_grid.view(B_ * self.num_heads, N, 2)


        k_grid = pos.view(B_ * self.num_heads, N, 2)


        displacement = (q_grid.unsqueeze(2) - k_grid.unsqueeze(1)) * 0.5
        # [B_*H, N, N, 2]

        rpb = self.relative_position_bias_table.view(
            2 * Wh - 1, 2 * Ww - 1, self.num_heads
        ).permute(2, 0, 1).contiguous()   # [heads, 2Wh-1, 2Ww-1]

        rpb = rpb.unsqueeze(0).expand(B_, -1, -1, -1).reshape(
            B_ * self.num_heads, 1, 2 * Wh - 1, 2 * Ww - 1
        )

        displacement = displacement.to(rpb.dtype)
        attn_bias = F.grid_sample(
            rpb,
            displacement[..., (1, 0)],   # grid_sample expects (x, y)
            mode='bilinear',
            align_corners=True
        )   # [B_*heads, 1, N, N]
    ...
    def forward(self, x, mask: Optional[torch.Tensor] = None):
        ...
        qkv = self.qkv(x)
        q0, k0, v0 = qkv.chunk(3, dim=-1)   # [B_, N, C]

        q = q0.view(B_, N, self.num_heads, self.head_dim).permute(0, 2, 1, 3)  # [B_, h, N, d]

        q_map = q0.transpose(1, 2).contiguous().view(B_, C, Wh, Ww)
        k_map = k0.transpose(1, 2).contiguous().view(B_, C, Wh, Ww)
        v_map = v0.transpose(1, 2).contiguous().view(B_, C, Wh, Ww)


        q_map_h = q_map.view(B_, self.num_heads, self.head_dim, Wh, Ww)
        k_map_h = k_map.view(B_, self.num_heads, self.head_dim, Wh, Ww)
        v_map_h = v_map.view(B_, self.num_heads, self.head_dim, Wh, Ww)

        q_map_h = q_map_h.reshape(B_ * self.num_heads, self.head_dim, Wh, Ww)
        k_map_h = k_map_h.reshape(B_ * self.num_heads, self.head_dim, Wh, Ww)
        v_map_h = v_map_h.reshape(B_ * self.num_heads, self.head_dim, Wh, Ww)

        offset = self.conv_offset(q_map_h)   # [B_*h, 2, Wh, Ww]

        step_y = 2.0 / max(Wh - 1, 1)
        step_x = 2.0 / max(Ww - 1, 1)
        ...
        offset = torch.tanh(offset) * offset_range * self.offset_range_factor
        offset = offset.permute(0, 2, 3, 1).contiguous()   # [B_*h, Wh, Ww, 2]

        ref = self._get_window_grid(B_ * self.num_heads, offset.dtype, offset.device)
        pos = (ref + offset).clamp(-1.0, 1.0)   # sampled key/value positions
        pos = pos.to(k_map_h.dtype)
        ...
        k = k_sample.view(B_, self.num_heads, self.head_dim, N).permute(0, 1, 3, 2)  # [B_, h, N, d]
        v = v_sample.view(B_, self.num_heads, self.head_dim, N).permute(0, 1, 3, 2)

        # 6) attention logits = qk + continuous RPB
        attn = (q * self.scale) @ k.transpose(-2, -1)   # [B_, h, N, N]
        attn = attn + self._get_continuous_rpb(B_, pos)
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
             ...
             act_layer=nn.GELU, norm_layer=nn.LayerNorm, use_deform_attn=False):
        super().__init__()
        self.dim = dim
...
class BasicLayer(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
                 ...
                 drop_path=0., norm_layer=nn.LayerNorm, downsample=None, use_checkpoint=False, deform_block_ids=None):
        ...
        self.blocks = nn.ModuleList([
            SwinTransformerBlock(
                ...
                use_deform_attn=(i in deform_block_ids)
            )
            for i in range(depth)
        ])
...
class SwinTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
        ...
        #
        self.num_features_0 = int(embed_dim * 2 ** (self.num_layers - 4))
        self.num_features_1 = int(embed_dim * 2 ** (self.num_layers - 3))
        self.num_features_2 = int(embed_dim * 2 ** (self.num_layers - 2))
        #
        ...
        for i_layer in range(self.num_layers):
            ...
            if i_layer == 2 and depths[i_layer] >= 18:
                deform_block_ids = [10, 12, 14, 16]  # block 11 13 15 17 without shift window
            ...
            layers += [BasicLayer(
                ...
                downsample=PatchMerging if (i_layer < self.num_layers - 1) else None,
                use_checkpoint=use_checkpoint,
                deform_block_ids=deform_block_ids,
            )]
        ...
        #
        self.norm_0 = norm_layer(self.num_features_0)
        self.norm_1 = norm_layer(self.num_features_1)
        self.norm_2 = norm_layer(self.num_features_2)
        #
    ...
    def forward_features(self, x):
        ...
        for layer in self.layers:
            x, stage_x = layer(x)
            nums_x.append(stage_x)

        nums_x[0] = self.norm_0(nums_x[0])
        nums_x[1] = self.norm_1(nums_x[1])
        nums_x[2] = self.norm_2(nums_x[2])
        nums_x[3] = self.norm(nums_x[3])

        #for i, num_x in enumerate(nums_x):
        #    nums_x[i] = self.norm(num_x)
